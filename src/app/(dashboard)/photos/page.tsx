...
export default function PhotosPage() {
  ...
  const loadUserData = async () => {
    ...
    setCanUpload(adminRoles.includes(userData?.role || ''))
    setUserDeptId(userData?.department_id || null)
    setDepartments(deptData || [])

    if (!uploadForm.department_id && deptData?.[0]) {
      setUploadForm(prev => ({ ...prev, department_id: userData?.department_id || deptData[0].id }))
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setError(`예외 발생: ${err?.message || '알 수 없는 오류'}`)
    } finally {
      setUploading(false)
    }
  }
